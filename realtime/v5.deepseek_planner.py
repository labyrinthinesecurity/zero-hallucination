...
def generate_robot_plan():
    grid_size = 30
    num_robots = 10
    T = 36  # Increased timesteps for more complex coordination
    ...
    for i, name in enumerate(robot_names):
        ...
        for t in range(1, T):
            ...
            if name == 'BotH':
                # BotH: Go right to x=4, then down to y=15 (avoid y=13 area), then right to goal
                if current_x < 4:
                    next_x = (current_x + 1) % grid_size
                elif current_x == 4 and current_y == 14:
                    next_y = (current_y + 1) % grid_size  # Move up to y=15 (avoid going down to y=13)
            ...
            # Special handling for BotG - no waiting, just different path
            elif name == 'BotG':
                # BotG: Go right to x=5, then up to y=14, then right to goal
                if current_x < 5:
                    next_x = (current_x + 1) % grid_size
                elif current_x == 5 and current_y == 12:
                    next_y = (current_y + 1) % grid_size  # Move up to y=13
                elif current_x == 5 and current_y == 13:
                    next_y = (current_y + 1) % grid_size  # Move up to y=14 (avoid staying at y=13)
